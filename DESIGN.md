...
| 2026-09-08 | Sold-out tracked items stay visible in the gallery with a "Sold Out" pill (`.sold-out-pill`, ghost-style against the hover scrim) instead of being dropped entirely; new `soldOut` field added to the `/api/gallery` payload and threaded through `gallery.js` | Robby's direction ("if something is sold out but tagged as website, it should still show as well and show that its sold out") |
| 2026-09-08 | `soldOut` also reads `item_variation_data.location_overrides[].sold_out` for the configured location, not just quantity-tracked inventory hitting 0 | First pass of the "Sold Out" pill only checked `track_inventory` + a live inventory count of 0. Wave Teapot still showed as available after Robby set its Status to "Sold out" in the Square dashboard, because that dropdown sets a manual per-location `sold_out` override — a completely separate mechanism from inventory-count tracking, unrelated to the Inventory API. Both are now treated as sold out |
